...
const TAB_DEFS = [
  { label: 'Arms',    key: 'weapons',    filters: ['All','Sword','Lance','Axe','Bow','Tome','Light','Staff','Scroll'] },
  { label: 'Skills',  key: 'skills',     filters: ['All','Passive','Aura','Combat','Attack','Defend','Turn','Action'] },
...
export class CompendiumOverlay {
  ...
  _getItemsForTab(tabIndex) {
    ...
    if (!gd) return [];
    switch (def.key) {
      case 'weapons': return gd.weapons || [];
      case 'skills': return gd.skills || [];
      case 'weaponArts': return gd.weaponArts?.arts || [];
      case 'classes': return gd.classes || [];
      case 'items': return [
        ...(gd.consumables || []),
        ...(gd.accessories || []),
        ...(gd.whetstones || []),
      ];
      case 'lords': return gd.lords || [];
      case 'blessings': return gd.blessings?.blessings || [];
      case 'terrain': return gd.terrain || [];
      case 'affixes': return gd.affixes?.affixes || [];
      default: return [];
    }
  }
  ...
  _draw() {
    ...
    // Title
    const title = this.scene.add.text(left + 20, top + 16, 'COMPENDIUM', {
  ...
  _drawSearch(left, top, panelW) {
    ...
    const searchBox = this.scene.add.rectangle(searchBoxX, searchBoxY, searchBoxW, searchBoxH, 0x111111, 1)
      .setDepth(DEPTH_UI).setStrokeStyle(1, this.searchInputActive ? 0xffdd44 : 0x555555)
      .setInteractive({ useHandCursor: true });
    searchBox.on('pointerdown', () => { this.searchInputActive = true; this._draw(); });
    ...
    searchValue.on('pointerdown', () => { this.searchInputActive = true; this._draw(); });
  ...
  _drawTabs(left, top, panelW) {
    ...
    for (let i = 0; i < TAB_DEFS.length; i++) {
      ...
      const tabText = this.scene.add.text(tx, tabY, TAB_DEFS[i].label, {
  ...
  _drawFilters(left, filterY, panelW, filters) {
    ...
    for (let i = 0; i < filters.length; i++) {
      ...
      const chip = this.scene.add.text(fx, filterY, filters[i], {
  ...
  _renderItems(items, startY, left, panelW) {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const item = items[i];
      switch (def.key) {
        case 'weapons': this._renderWeapon(item, y, left, rightX); break;
        case 'skills': this._renderSkill(item, y, left, rightX); break;
        case 'weaponArts': this._renderArt(item, y, left, rightX); break;
        case 'classes': this._renderClass(item, y, left, rightX); break;
        case 'items': this._renderItem(item, y, left, rightX); break;
        case 'lords': this._renderLord(item, y, left, rightX); break;
        case 'blessings': this._renderBlessing(item, y, left, rightX); break;
        case 'terrain': this._renderTerrain(item, y, left, rightX); break;
        case 'affixes': this._renderAffix(item, y, left, rightX); break;
      }
    }
  }

  _renderWeapon(item, y, left, rightX) {
    const nameColor = this._matchesSearch(item.name) ? '#66ff66'
      : (item.tier === 'Legend' ? '#ffdd44' : '#e0e0e0');
    this._text(left + 25, y, item.name, nameColor);
    this._text(rightX, y, `${item.type}  ${item.tier || ''}  ${item.rankRequired || ''}`, '#888888', 1);
  ...
  _renderArt(item, y, left, rightX) {
    const nameColor = this._matchesSearch(item.name) ? '#66ff66' : '#e0e0e0';
    this._text(left + 25, y, item.name, nameColor);
    this._text(rightX, y, `${item.weaponType || ''}  ${item.requiredRank || ''}  HP:${item.hpCost ?? '?'}  Act:${item.unlockAct || '?'}`, '#888888', 1);
    this._text(left + 25, y + 14, item.description || '', '#aaaaaa');
  }
  ...
  _renderClass(item, y, left, rightX) {
    ...
    const profs = Array.isArray(item.weaponProficiencies)
      ...
      : (typeof item.weaponProficiencies === 'string' ? item.weaponProficiencies : '');
  ...
  _renderItem(item, y, left, rightX) {
    ...
    if (item.combatEffects) {
      ...
      desc = Object.entries(item.effects).map(([k, v]) => `${k}+${v}`).join(', ');
    } else if (item.effect) {
  ...
  _renderLord(item, y, left, rightX) {
    ...
    this._text(left + 25, y + 28, `Skill: ${item.personalSkill || '-'}  Weapon: ${weapon}`, '#aaaaaa');
  }
  ...
  _renderTerrain(item, y, left, rightX) {
    ...
    this._text(left + 25, y + 14, `Move \u2014 Inf:${mc.Infantry ?? '-'} Arm:${mc.Armored ?? '-'} Cav:${mc.Cavalry ?? '-'} Fly:${mc.Flying ?? '-'}${special}`, '#aaaaaa');
  }
  ...
  _drawPageNav(left, top, panelW, panelH, totalPages) {
    ...
    // Page indicator
    const pageInd = this.scene.add.text(cx, navY, `Page ${this.currentPage + 1}/${totalPages}`, {
      fontFamily: 'monospace', fontSize: '10px', color: '#888888',
    }).setOrigin(0.5).setDepth(DEPTH_UI);
    this.objects.push(pageInd);

    if (this.currentPage > 0) {
      const prevBtn = this.scene.add.text(cx - 80, navY, '\u25C0 Prev', {
    ...
    if (this.currentPage < totalPages - 1) {
      const nextBtn = this.scene.add.text(cx + 80, navY, 'Next \u25B6', {
        fontFamily: 'monospace', fontSize: '11px', color: '#aaaaaa',
        backgroundColor: '#333333', padding: { x: 8, y: 3 },
      }).setOrigin(0.5).setDepth(DEPTH_UI).setInteractive({ useHandCursor: true });
