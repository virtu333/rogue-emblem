...
export class CampaignMapOverlay {
  ...
  _draw() {
    ...
    // Dark fullscreen bg
    const bg = this.scene.add.rectangle(cx, cy, 640, 480, 0x000000, 0.7)
    ...
    const panelX = cx;
    const panelY = cy;
    const panel = this.scene.add.rectangle(panelX, panelY, PANEL_W, PANEL_H, 0x1a1a2e, 1)
    ...
    const titleText = actNum ? `${actNum}: ${actConfig.name || ''}` : (actConfig.name || 'Campaign Map');
    const title = this.scene.add.text(panelX, panelTop + 18, titleText, {
      fontFamily: 'monospace', fontSize: '14px', color: '#ffdd44',
    }).setOrigin(0.5).setDepth(DEPTH_UI);
    this.objects.push(title);

    // Close button
    const closeBtn = this.scene.add.text(panelLeft + PANEL_W - 16, panelTop + 8, '[X]', {
    ...
    for (const node of nodes) {
      ...
      for (const edgeId of (node.edges || [])) {
        ...
        const isActivePath = (fromState === 'active' && toState === 'next') ||
                             (fromState === 'completed' && (toState === 'active' || toState === 'completed'));
        graphics.lineStyle(isActivePath ? 2 : 1, isActivePath ? 0xffdd44 : 0x666666, isActivePath ? 0.9 : 0.3);
    ...
    for (const node of nodes) {
      ...
      const circle = this.scene.add.circle(pos.x, pos.y, NODE_RADIUS, color, alpha)
        .setDepth(DEPTH_UI);
      ...
      const iconText = this.scene.add.text(pos.x, pos.y, icon, {
        fontFamily: 'monospace', fontSize: '9px', color: iconColor,
      }).setOrigin(0.5).setDepth(DEPTH_UI).setAlpha(state === 'locked' ? 0.5 : 1);
      this.objects.push(iconText);

      // "YOU" marker for active node
      if (state === 'active') {
        const marker = this.scene.add.text(pos.x, pos.y - NODE_RADIUS - 10, '\u25BC YOU', {
          fontFamily: 'monospace', fontSize: '8px', color: '#44ff44',
        }).setOrigin(0.5).setDepth(DEPTH_UI);
  ...
  _drawLegend(cx, y) {
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      const dot = this.scene.add.circle(ex - 14, y, 4, parseInt(entries[i].color.replace('#', ''), 16))
        .setDepth(DEPTH_UI);
      if (entries[i].label === 'Locked') dot.setAlpha(0.4);
      this.objects.push(dot);

      const lbl = this.scene.add.text(ex, y, entries[i].label, {
        fontFamily: 'monospace', fontSize: '8px', color: entries[i].color,
