...
// isolated instance) and the key transitions, rendered through the runtime
...
export function layoutNames(rows, fill, fallback = 'Plain') {
  return rows.map((row) => [...row].map((ch) => (ch === 'X' ? fill : (CODE[ch] ?? fallback))));
}
...
const TRANSITIONS = [
  [
    'Shoreline: convex/concave corners, island, bank face + foam',
    'grassland',
    ['..........', '..~~~~~...', '.~~~~~~~..', '.~~~..~~~.', '..~~~~~~..', '....~~....'],
  ],
  [
  ...
  [
    'Forest block + mountain range + village',
    'grassland',
    ['..FFF..MM.', '.FFFFF.MMM', '.FFF...MM.', '..F..V....', '......f...', '::....FF..'],
  ],
  [
    'Swamp / bog / acid patchwork (cell-by-cell alternation)',
    'swamp',
    ['..sbsb....', '.sbsAsb.bb', '.bsAAbsbab', '..sbs.bsb.', '.F....b...', 'FF........'],
  ],
  [
...
function renderLayout(names, biome, seed) {
  const img = resultImage(renderBattlefieldTerrain({ names, biome, seed }));
  return { data: upscaleNearest(img.data, img.w, img.h, SHOW), w: img.w * SHOW, h: img.h * SHOW };
}
...
export async function buildSheet(path, seed = 4242) {
  ...
  for (const [k, [title, terrain, biome, layout]] of PANELS.entries()) {
    ...
    const img = renderLayout(layoutNames(layout, terrain, fallback), biome, seed + k * 17);
  ...
  for (const [k, [title, biome, layout]] of TRANSITIONS.entries()) {
    ...
    const img = renderLayout(layoutNames(layout, 'Plain', fallback), biome, seed + 900 + k * 31);
    const x = pad + col * (img.w + pad);
