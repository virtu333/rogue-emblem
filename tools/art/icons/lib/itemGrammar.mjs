...
const ELEMENT_COVER = { fire: 'blood', wind: 'verdigris', thunder: 'sky', dark: 'unlight', light: 'parchment' };
const ELEMENT_EMBLEM = { fire: 'ember', wind: 'leaf', thunder: 'pearl', dark: 'lilac', light: 'gilt' };
...
export function weaponSpec(w) {
  ...
  switch (w.type) {
    case 'Sword': {
      const variant = sp.brave ? 'twin' : /rapier/i.test(w.name) ? 'rapier' : sp.throw ? 'short' : sp.reaver ? 'serrated' : /wo dao|killing/i.test(w.name) ? 'curved' : 'broad';
      const blade = sp.drain && tier !== 'Legend' ? 'blackened' : t.metal;
      return D.sword({
        blade,
        fit: t.fit,
        grip: tier === 'Legend' ? 'blood' : tier === 'Rare' ? 'unlight' : tier === 'Silver' ? 'cloth' : 'cord',
        gem: sp.crit || sp.effective ? acc : tier === 'Legend' ? 'blood' : null,
        core: sp.magic || sp.poison || sp.drain || sp.sunder || tier === 'Legend' ? acc || 'ember' : null,
        variant,
      });
    }
    case 'Lance':
      return D.lance({
        ...
        variant: sp.throw ? 'javelin' : sp.crit || sp.reaver ? 'barbed' : sp.brave || sp.effective || tier === 'Legend' ? 'broad' : 'leaf',
      });
    case 'Axe':
      return D.axe({
        head: t.metal,
        ...
        variant: /hammer/i.test(w.name) ? 'hammer' : sp.throw ? 'hand' : sp.brave || sp.reaver ? 'double' : tier === 'Legend' ? 'great' : 'bearded',
      });
    case 'Bow':
      return D.bow({
        limb: tier === 'Silver' ? 'silver' : tier === 'Legend' ? 'gilt' : tier === 'Rare' ? 'blackened' : tier === 'Steel' ? 'darkWood' : 'wood',
        fit: acc || t.fit,
    ...
    case 'Tome': {
      ...
      return D.tome({ cover: ELEMENT_COVER[el], fit: t.fit, emblem: el, emblemMat: ELEMENT_EMBLEM[el] });
    }
    case 'Light':
      return D.tome({ cover: 'parchment', fit: t.fit, emblem: 'light', emblemMat: sp.crit ? 'blood' : 'gilt', pages: 'pearl' });
    case 'Staff': {
      const n = w.name.toLowerCase();
      if (/sleep|silence/.test(n)) return D.staff({ rod: 'darkWood', fit: 'blackened', gem: 'unlight', variant: 'crescent' });
      if (/rescue|warp/.test(n)) return D.staff({ rod: 'darkWood', fit: t.fit === 'blackened' ? 'gilt' : t.fit, gem: 'sky', variant: 'wing' });
      if (/restore/.test(n)) return D.staff({ fit: t.fit, gem: 'pearl', variant: 'crescent' });
      return D.staff({ rod: tier === 'Iron' ? 'wood' : 'darkWood', fit: tier === 'Iron' ? 'bronze' : t.fit, gem: 'verdigris' });
    }
    case 'Breath':
      return D.stoneGem({ mat: sp.poison ? 'verdigris' : /ancient/i.test(w.name) ? 'unlight' : 'blood', fit: t.fit });
    case 'Scroll':
...
const BOOSTER = {
  ...
  SKL: () => D.tome({ cover: 'pearl', fit: 'gilt', emblem: 'dark', emblemMat: 'ink', pages: 'parchment' }),
...
export function consumableSpec(c) {
  switch (c.effect) {
    ...
    case 'reclass':
      return D.seal({ wax: c.subEffect === 'mounted' ? 'earth' : 'steel', ribbon: 'cloth', metal: 'silver', glyph: c.subEffect === 'mounted' ? 'horseshoe' : 'boot' });
    case 'statBoost':
...
const RING_STAT = { STR: 'blood', MAG: 'unlight', SPD: 'sky', DEF: 'steel', RES: 'lilac', SKL: 'pearl' };
...
export function accessorySpec(a) {
  ...
  if (/ Ring$/.test(n)) {
    if (keys.length === 1 && RING_STAT[keys[0]]) return D.ringItem({ band: 'gilt', gem: RING_STAT[keys[0]] });
  ...
  const table = {
    ...
    'Delphi Shield': () => D.shield({ face: 'lilac', rim: 'gilt', emblem: 'diamond', emblemMat: 'pearl' }),
    "Veteran's Crest": () => D.medal({ ribbon: 'steel', disc: 'bronze', glyph: 'star', glyphMat: 'gilt', shape: 'shield' }),
    'Wrath Band': () => D.band({ metal: 'blackened', inlay: 'blood' }),
    'Counter Seal': () => D.medal({ ribbon: 'slate', disc: 'silver', glyph: 'cross', glyphMat: 'steel' }),
    'Forest Charm': () => D.pendant({ form: 'leaf', body: 'leaf', chain: 'wood', gem: 'verdigris' }),
    ...
    'Recoil Guard': () => D.shield({ face: 'blood', rim: 'silverFit', emblem: 'cross', emblemMat: 'silver' }),
    "Bounty Hunter's Mark": () => D.medal({ ribbon: 'blood', disc: 'bronze', glyph: 'eye', glyphMat: 'blood', shape: 'diamond' }),
    'Moontide Amulet': () => D.pendant({ form: 'moon', body: 'silver', chain: 'silverFit' }),
    "Gambler's Coin": () => D.coin({ face: 'gilt', back: 'unlight' }),
    'Vanguard Crest': () => D.medal({ ribbon: 'blood', disc: 'silver', glyph: 'star', glyphMat: 'blood', shape: 'shield' }),
    'Diamond Medallion': () => D.medal({ ribbon: 'steel', disc: 'silver', glyph: 'diamond', glyphMat: 'sky', shape: 'diamond' }),
    "Hunter's Cloak": () => D.cloak({ cloth: 'verdigris', clasp: 'bronze' }),
    "Duelist's Glove": () => D.glove({ leather: 'blood', cuff: 'gilt' }),
    'Warding Charm': () => D.pendant({ form: 'oval', body: 'silver', gem: 'lilac', chain: 'silverFit' }),
...
const WHET = { choice: ['pearl', true], might: ['blood'], crit: ['ember'], hit: ['pearl'], weight: ['sky'] };
export function whetstoneSpec(w) {
  const [streak, silver] = WHET[w.forgeStat] || ['blood'];
  return D.whetstone({ streak, silver: !!silver });
}

const IMBUE_MAT = { vampiric: 'blood', armorbane: 'steel', keen: 'ember', venom: 'verdigris', binding: 'earth', warded: 'lilac' };
...
export function transformSpec(spec, k, dx = 0, dy = 0) {
  ...
  return {
    ...
    glints: spec.glints?.map((g) => Object.assign([g[0] * k + dx, g[1] * k + dy], { minSize: g.minSize })),
  };
}
...
export function withBadge(spec, kind, mat = 'ember') {
  ...
  if (kind === 'growth')
    parts.push({ shape: { kind: 'poly', pts: [[25, 19.5], [31, 25.5], [27.6, 25.5], [27.6, 31], [22.4, 31], [22.4, 25.5], [19, 25.5]] }, mat, shade: 'bevel', bevel: 0.6, sep: true, spec: 1, emissive: true });
  else if (kind === 'plus')
    parts.push({ shape: { kind: 'poly', pts: [[23.4, 19.5], [26.6, 19.5], [26.6, 23.4], [30.5, 23.4], [30.5, 26.6], [26.6, 26.6], [26.6, 30.5], [23.4, 30.5], [23.4, 26.6], [19.5, 26.6], [19.5, 23.4], [23.4, 23.4]] }, mat, shade: 'bevel', bevel: 0.6, sep: true, spec: 1, emissive: true });
  return { ...base, parts };
}

/** A boon seen through the Hollow Sun: item icon inside a corona (blessings). */
export function haloed(spec, tier = 1) {
  const halo = D.hollowSun({ corona: tier >= 4 ? 'ember' : tier >= 3 ? 'gilt' : tier >= 2 ? 'bronze' : 'silver', core: 'ink' });
...
export function upgradeSpec(u) {
  ...
  if (u.category === 'recruit_stats') return badge(D.helm({ metal: 'steel', plume: statMat || 'blood' }));
  if (u.category === 'lord_bonuses') {
    if (/vision|revelation|glimpse/.test(`${u.id} ${u.name}`.toLowerCase())) return D.hollowSun({ corona: 'gilt' });
    ...
    if (/commander|partner/.test(u.id)) return D.banner({ cloth: 'steel', emblem: 'cross', emblemMat: 'gilt' });
  ...
  if (u.category === 'capacity') {
    ...
    if (/armory|arms|master_of_arms|forge/.test(u.id)) return D.anvil({ metal: /silver/.test(u.id) ? 'silver' : 'iron' });
  ...
  if (u.category === 'starting_skills') return D.scroll({ cord: /art/.test(u.id) ? 'ember' : 'unlight', seal: /art/.test(u.id) ? 'gilt' : 'unlight' });
  return D.chest();
}
...
const BLESSING_BOON = {
  ...
  forbidden_tome: () => D.tome({ cover: 'unlight', fit: 'gilt', emblem: 'dark', emblemMat: 'lilac' }),
  ...
  focused_curriculum: () => D.tome({ cover: 'sky', fit: 'silverFit', emblem: 'thunder', emblemMat: 'pearl' }),
};
...
export function catalog(data) {
  ...
  groups.push({
    ...
    items: [scrolls.find((s) => !s.teachesWeaponArtId), scrolls.find((s) => s.teachesWeaponArtId)]
      .filter(Boolean)
      .map((w) => ({ name: w.teachesWeaponArtId ? 'Weapon-art scroll' : 'Skill scroll', spec: weaponSpec(w) })),
  });
  groups.push({ label: 'Consumables', items: data.consumables.map((c) => ({ name: c.name, spec: consumableSpec(c) })) });
  groups.push({ label: 'Accessories', items: data.accessories.map((a) => ({ name: a.name, spec: accessorySpec(a) })) });
  groups.push({
    label: 'Forge',
    items: [
  ...
  groups.push({ label: 'Blessings', items: data.blessings.blessings.map((b) => ({ name: b.name, spec: blessingSpec(b) })) });
  const mu = data.metaUpgrades.upgrades || data.metaUpgrades;
  groups.push({ label: 'Upgrades', items: mu.map((u) => ({ name: u.name, spec: upgradeSpec(u) })) });
  return groups;
}
