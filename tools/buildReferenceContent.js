...
function buildReference() {
  ...
  const rankRequirements = reference.weaponRanks.displayOrder.map((rank) => ({
    ...
    legendaryWeapons: weapons.filter(
      (weapon) =>
        weapon.rankRequired === rank && weapon.tier === legendaryTier && weapon.type !== 'Scroll',
    ).length,
  }));
  ...
  const rules = {
    promotedMastery:
      reference.help.notes.find((line) => line.toLowerCase().includes('promoted units')) || '',
    forecastAs:
      reference.help.notes.find((line) => line.toLowerCase().includes('forecast as')) || '',
  };
  ...
  const helpPages = [
    ...
    },
    {
      ...
      lines: [
        {
          text: `Weapon ranks: ${reference.weaponRanks.displayOrder.join(' -> ')}`,
          color: '#ffdd44',
        },
        ...
        { text: '' },
        {
          text: `Legendary weapons require ${reference.weaponRanks.legendaryRequires}.`,
          color: '#66ff66',
        },
