...
phrases, F to E and then G, F, E. It is reiteration, not a held note: each
syllable is let go early and sung dry enough to be heard as a new syllable,
and the celli (plucked violas in the calm mix) give the syllables their
consonants. Under it a stepwise Phrygian stab bass rises A, B-flat, C, D,
one accented chord a bar and every other bar empty, so the same E is the
fifth of A minor, the raised fourth over B-flat, the third of C and the
second over D (God-Shattering Star's chant over its rising stabs, as
verified). The chords that carry the steps keep F away from the reciting
tone.
...
leading tone, and stops; the hymn belongs to the horns and trumpets in
unison, over trombones hammering the chords (in eighths, with an eighth's
breath at the end of each bar: at sixteenths the library's staccato
trombones stop sounding like brass). The hymn is the zealots' song to the
...
bar where A should be is silent. The loop's first stab is that A, from
the B-flat above it (bII7 to i, never V to i); the intro is the same
B-flat seven swelling.
...
28-43 (the hymn in F; the violins join an octave up for its second half)
| R 44-51 (the choir's reprise and the Hollow Sun). Loop 3-51, 49 bars.
...
leading tone, intended. Sample zones are levelled along every line with
`even` (from battle_elite_act2).
"""
...
def build():
    ...
    # ================================================================ the recitation
    # (dry enough, and each syllable let go early, so the reiteration is heard
    # as reiteration and not as a held note)
    chw = b.part('ch_w', 'choir', role='lead', layer='full', reverb=0.42, gain=1, humanize_ms=16,
                 vel_jitter=0.1)
    chm = b.part('ch_m', 'choir', role='lead2', layer='full', reverb=0.42, gain=1, humanize_ms=16,
                 vel_jitter=0.1)
    cho = b.part('c_ch', 'oohs', role='lead', layer='calm', reverb=0.42, humanize_ms=16)
    for bar, dyn in ((A1, 'f'), (A2, 'f')):
        chw.at(bar).play(f'@{dyn} ' + CHANT, gate=0.8)
        chm.at(bar).play(f'@{dyn} ' + CHANT, transpose=-12, gate=0.8)
        cho.at(bar).play('@mf ' + CHANT, gate=0.8)
    ...
    b.timp('P', '@f E2q rq E2q rq | E2q rq E2e E2e E2q | F2q rq F2q rq | E2q rq E2e E2e E2q |'
    ...
    b.hit(H0, pieces=('crash', 'bd', 'gong'), vel=0.95)
    ...
    # the hymn: horns and trumpets in unison, tongued, not slurred (every
    # note of it is struck), over trombones hammering the chord below them
    b.lead('H', HYMN, inst='trumpets', name='hymn_tpt', dyn='ff', layer='full', gain=3,
           legato=False, humanize_ms=14, vel_jitter=0.08)
    ...
    bass(ebp, H, chart(CH_H), 'e e e e e e e e', 'b b b b b b b b', floor=28, vel=0.74,
         accents='> - - - > - - -')
    # the violins join the hymn an octave up for its second half, and its climb
    vt = b.part('h_vn', 'violins', role='lead2', layer='full', art='sus', gain=2)
    vt.at(H + 8).play('@ff ' + '|'.join(HYMN.strip().split('|')[8:]), transpose=12)
    ...
    tmp.at(R).play('%default @ff A2q rq rh | A2q rq rh | Bb2q rq C3q rq | F2q rq rh |'
                   ' F2q rq Bb2q rq | C3q rq D3q rq | %roll Bb2w~ | Bb2w |')
    ...
    kit.play(R + 7, {'kick': 'x...............', 'tom_lo': '........x.x.xxxx'}, vel=0.7, ramp=0.4)
    for bar in (A2, P, H, R):
