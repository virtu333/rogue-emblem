  ...
  async run(maxActions = 5000) {
    ...
    try {
      ...
      if (battleParams.nodeId == null || battleParams.nodeId === '') {
        battleParams.nodeId = String(
          this.fixture.id || `${battleParams.act || 'battle'}:${battleParams.objective || 'rout'}`,
        );
      }
      ...
      const replay = {
        ...
        seed: this.seed,
        scenarioId: this.fixture.id,
        resolvedTemplateId: driver?.battle?.battleConfig?.templateId || null,
