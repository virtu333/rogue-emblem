...
describe('canPromote', () => {
  ...
  it('Chevalier promotion to Holy Knight gains Light proficiency', () => {
    ...
    expect(unit.proficiencies).toEqual(
...
describe('createRecruitUnit', () => {
  ...
  it('returns unit with faction npc', () => {
    const unit = createRecruitUnit(
      { className: 'Fighter', name: 'Galvin', level: 3 },
      fighterClass,
      data.weapons,
    );
  ...
  it('has correct class, level, and name', () => {
    const unit = createRecruitUnit(
      { className: 'Fighter', name: 'Galvin', level: 5 },
      fighterClass,
      data.weapons,
    );
  ...
  it('has a weapon appropriate to level', () => {
    const low = createRecruitUnit(
      { className: 'Fighter', name: 'Galvin', level: 2 },
      fighterClass,
      data.weapons,
    );
    ...
    const mid = createRecruitUnit(
      { className: 'Fighter', name: 'Galvin', level: 8 },
      fighterClass,
      data.weapons,
    );
  ...
  describe('Longbow secondary weapon for archer/sniper recruits', () => {
    ...
    it('gives Archer recruit a Longbow as secondary weapon', () => {
      const unit = createRecruitUnit(
        { className: 'Archer', name: 'Wil', level: 3 },
        archerClass,
        data.weapons,
      );
    ...
    it('high-level Archer still gets Longbow alongside Silver Bow', () => {
      const unit = createRecruitUnit(
        { className: 'Archer', name: 'Shinon', level: 14 },
        archerClass,
        data.weapons,
      );
      expect(unit.inventory.length).toBe(2);
    ...
    it('does NOT give Longbow to non-archer bow class', () => {
      const unit = createRecruitUnit(
        { className: 'Assassin', name: 'Shade', level: 8 },
        assassinClass,
        data.weapons,
      );
      const longbow = unit.inventory.find((w) => w.name === 'Longbow');
...
describe('calculateCombatXP tiered diminishing returns', () => {
  ...
  it('gives bonus for under-leveled attacker', () => {
    expect(calculateCombatXP(unit(3), unit(6), false)).toBe(
      XP_BASE_COMBAT + 3 * XP_LEVEL_DIFF_SCALE,
    );
  });

  it('normal scale for advantage 1-3 (no kill)', () => {
    expect(calculateCombatXP(unit(5), unit(4), false)).toBe(
      XP_BASE_COMBAT - 1 * XP_LEVEL_DIFF_SCALE,
    );
    expect(calculateCombatXP(unit(6), unit(3), false)).toBe(
      XP_BASE_COMBAT - 3 * XP_LEVEL_DIFF_SCALE,
    );
  });
  ...
  it('advantage 5 uses steep scale for 2 extra levels', () => {
    ...
    expect(xp).toBe(
      Math.max(XP_MIN, XP_BASE_COMBAT - 3 * XP_LEVEL_DIFF_SCALE - 2 * XP_LEVEL_DIFF_STEEP),
    );
  });
