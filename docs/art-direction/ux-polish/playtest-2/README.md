...
| | |
|---|---|
| `threat-1280.png` | Desktop: hovering a destination. Each enemy that can reach it gets a small crimson eye and a thin red line to the tile (no damage numbers, nothing drawn on the tile); the info panel reads `Threat: 4 foes can reach`. |
