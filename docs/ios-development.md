...
`.github/workflows/testflight.yml` builds and uploads a TestFlight build on demand: Actions → **TestFlight** → **Run workflow** (pick the branch, usually `main`). It runs on a macOS runner with the latest stable Xcode: `npm ci`, `npm run ios:sync`, then an unsigned `xcodebuild archive` and `-exportArchive` with `destination=upload`, which signs it for App Store distribution through Xcode's cloud-managed signing with the App Store Connect API key (no certificate or profile is stored in the repo or in secrets, and no registered device is needed: an automatically signed archive would need an iOS development profile, which Apple only issues once the team has a registered device). The archive is checked before upload (bundle ID, build number, `PrivacyInfo.xcprivacy` and the web game inside `App.app`); the build shows up in App Store Connect → TestFlight once Apple finishes processing.

- **Mode:** `upload` (default) sends the build to App Store Connect. `validate` builds, signs and exports the same App Store IPA, checks its distribution signature and profile, and runs App Store Connect's validation (`altool --validate-app`) without uploading — for trying out changes to the workflow. `status` builds nothing: it reports what App Store Connect did with the recent builds.
- **After an upload:** the run waits (up to 30 minutes) for Apple to process the build and puts the result in the run's summary (`tools/ios/buildStatus.mjs`): each recent build's version, Apple's processing result (`VALID`, or `INVALID`/`FAILED` when Apple rejected it) and its TestFlight state. A rejected build fails the run; one still processing after 30 minutes only warns, and `status` mode reports on it later. Validate runs print the same report without waiting.
- **Testers:** once Apple has processed an upload, the run sends it to the `testers` group (default **Public Playtest**, the public link; `none` skips this) with `tools/ios/distributeBuild.mjs`: it sets the build's "What to Test" (the `whats_new` input, or the commit title without its PR number), adds the build to the group and, for an external group, submits it for beta review. Testers get it once Apple approves it. If this fails (no such group, the build still processing after 30 minutes, a rejected review) the run fails with the reason; add the build by hand in App Store Connect → TestFlight → the group. Validate runs only check that the group exists.
- **Pull requests:** a pull request that changes `ios/`, `tools/ios/`, `capacitor.config.*` or the workflow runs it in `validate` mode (branches of this repository only; forks get no secrets).
- **CI gate:** a first job waits for the newest CI run on the exact commit and stops the build unless it passed (a commit just merged to `main` has its push run still going; the job waits up to 45 minutes). A commit CI never ran on (a branch without a pull request) cannot be built.
...
- **Build number:** one above the highest build App Store Connect already has for the app, rejected ones included since they use up their number (`tools/ios/nextBuildNumber.mjs`), never below the project's `CURRENT_PROJECT_VERSION`; the run's input can override it. The version (`MARKETING_VERSION`) comes from the project.
...
- **Logs:** `testflight-logs` (archive, export and validation logs) is attached to every run.
...
- **Native mirror.** WKWebView keeps `localStorage` in WebKit's website-data store; iOS can evict it under storage pressure, and a kill can drop WebKit's last unflushed writes. With the Filesystem plugin present, every `emblem_rogue_*` key written to `localStorage` is copied to `Library/emblem-rogue-saves/` (backed up with the device, never evicted) — debounced 0.6 s (at most 3 s), deletions immediately, and flushed on backgrounding. Each key is double-buffered (`<key>.a.txt` / `.b.txt`, header with sequence number and length). The plugin's `writeFile` is not atomic (and its `rename` deletes the destination first), so the buffer holding the newest complete record is never written: every write, including overlapping ones, goes to the other buffer, which becomes the newest only once the last write in flight succeeds — a torn or failed write keeps the previous copy. Before the game boots the copy is read back: if `localStorage` lost its sentinel key (evicted) every mirrored key is restored and deleted keys stay deleted; otherwise only a strictly newer run/meta save (by its monotonic `savedAt`) replaces the local copy, and a key missing locally is never resurrected. Deletions are ordered too: a tombstone records `deletedSavedAt`, the newest `savedAt` the mirror had seen live for that key (including saves a debounce coalesced away and the value read just before `removeItem`), so when WebKit lost the removal itself — the run ended or the slot was deleted, the tombstone reached disk, the process was killed before WebKit flushed — the launch finds the old save still in `localStorage` and removes it (its `savedAt` is at or below `deletedSavedAt`) before the reconcile could mirror it back over the tombstone; a save stamped after the deletion (a new run in that slot whose native write was lost) is kept and mirrored. Without both stamps — a tombstone written before the field existed, or a key whose payload has no `savedAt` (settings, flags, clock floors) — order cannot be proven and the local value is kept, as before. A read-back that fails or takes over 2.5 s boots without mirroring for that session, so it can never overwrite the native copy with a fresh-looking store; a folder that exists but cannot be listed counts as a failure, never as empty. After an eviction the sentinel is only re-set once every mirrored save is back: a save that could not be read or written back (quota) keeps its native copy and is restored on a later launch instead of being taken for a deletion.
