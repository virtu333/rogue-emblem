...
- **Mode:** `upload` (default) sends the build to App Store Connect. `validate` builds, signs and exports the same App Store IPA, checks its distribution signature and profile, and runs App Store Connect's validation (`altool --validate-app`) without uploading — for trying out changes to the workflow.
