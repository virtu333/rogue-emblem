...
**Texture atlases are one versioned set (2026-09-25).** The cache policy now lives in `tools/pwa/offlineCachePolicy.js` (imported by `vite.config.js` and the tests). Every texture atlas is precached as a whole — `fx_atlas.png` + `fx_atlas.json`, the node-medal sheet, the traced-sprite pages and the four PC-98 portrait atlases (~3 MB; precache is now ~7.7 MB) — because their frame tables are either a JSON file (fx) or compiled into the precached JS bundle. Before, `fx_atlas.json` matched no route at all (an offline reload lost every combat effect) and the atlas PNGs were stable-filename StaleWhileRevalidate, so after a deploy the first launch could draw an old image with new frames. Precaching installs the new image, frame data and code together and swaps them at activation. `dontCacheBustURLsMatching` is narrowed to Vite's flat hashed files: the plugin default (`/^assets\//`) also matched the game's own media, which would have precached the atlases with `revision: null` — never refetched on update. The image route now covers `assets/terrain/` (painted terrain sheets were uncached) and holds up to 3200 entries (the shipped set is ~2450; the old 2400 was already exceeded). `tests/OfflineCachePolicy.test.js` checks that every file under `public/` and every URL BootScene, the warmup and the lazy loaders request is covered; `tests/e2e/offline-atlas.spec.js` (release config) checks offline reopen and an A→B atlas upgrade against the production build.
